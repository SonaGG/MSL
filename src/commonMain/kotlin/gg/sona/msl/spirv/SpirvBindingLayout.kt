...
fun interface SpirvBindingLayout {
    ...
    companion object {
        ...
        val SeparateSets = SpirvBindingLayout { request ->
            if (request.argumentBuffer >= 0) return@SpirvBindingLayout DescriptorLocation(3 + request.argumentBuffer, request.mslIndex)
            when (request.kind) {
        ...
        fun singleSet(set: Int = 0, bufferBase: Int = 0, textureBase: Int = 32, samplerBase: Int = 64) = SpirvBindingLayout { request ->
            if (request.argumentBuffer >= 0) return@SpirvBindingLayout DescriptorLocation(set + 1 + request.argumentBuffer, request.mslIndex)
            when (request.kind) {
